"""One error, one vocabulary (ADR-0068 d5).

``GateError.kind`` is what a caller acts on; ``code`` is the server's JSON-RPC
number when there was one; ``detail`` carries the server's message, capped,
and never enters ``str(exc)`` so a log line cannot leak what a gate said about
a held row. The two tables here are the same ones the constellation app's
``rpc.ts`` uses; change both or neither.
"""
...
#: HTTP status -> kind, for the HTTP transports. Anything else is ``remote_error``.
